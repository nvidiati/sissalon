...
class BillingController extends AdminBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        $this->stripeSettings = PaymentGatewayCredentials::withoutGlobalScope(CompanyScope::class)->first();
        ...
        return DataTables::of($paypalData)
            ...
            ->addColumn('action', function ($row) {
                if ($row->method == 'Stripe' && $row->invoice_id) {
                    return '<div class="text-right"><a href="' . route('admin.stripe.invoice-download', $row->invoice_id) . '" class="btn btn-primary btn-circle waves-effect" onclick="this.blur()" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a></div>';
                }

                if ($row->method == 'Paypal') {
                    return '<div class="text-right"><a href="' . route('admin.paypal.invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" onclick="this.blur()" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a></div>';
                }

                if ($row->method == 'Razorpay') {
                    return '<div class="text-right"><a href="' . route('admin.billing.razorpay-invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" onclick="this.blur()" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a></div>';
                }

                if ($row->method == 'Offline') {
                    return '<div class="text-right"><a href="' . route('admin.billing.offline-invoice-download', $row->id) . '" class="btn btn-primary btn-circle waves-effect" onclick="this.blur()" data-toggle="tooltip" data-original-title="Download"><span></span> <i class="fa fa-download"></i></a></div>';
                }
    ...
    public function subscribe(Request $request)
    {
        ...
        if ($subcriptionCancel)
        {
            ...
            try {
                ...
                $generatedBy = User::withoutGlobalScopes([CompanyScope::class, 'active'])->whereNull('company_id')->first();
    ...
    public function selectPackage(Request $request, $packageID)
    {
        ...
        $this->stripeSettings = PaymentGatewayCredentials::withoutGlobalScope(CompanyScope::class)->whereNull('company_id')->first();
