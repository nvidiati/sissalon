...
class EmployeeGroupController extends AdminBaseController
{
    ...
    {
        abort_if(!$this->user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('read_employee_group'), 403);
        ...
        if(\request()->ajax()){
            ...
            return \datatables()->of($employeeGroup)
                ->addColumn('action', function ($row) {
                    ...
                    if ($this->user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('update_employee_group')) {
                        $action .= '<a href="' . route('admin.employee-group.edit', [$row->id]) . '" class="btn btn-primary btn-circle"
                          data-toggle="tooltip" data-original-title="'.__('app.edit').'"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
                    }

                    if ($this->user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('delete_employee_group')) {
    ...
    {
        abort_if(!$this->user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('create_employee_group'), 403);
    ...
    {
        abort_if(!$this->user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('update_employee_group'), 403);
    ...
    {
        abort_if(!$this->user->roles()->withoutGlobalScopes()->latest()->first()->hasPermission('delete_employee_group'), 403);
