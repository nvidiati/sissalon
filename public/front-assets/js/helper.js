(function($) {
    ...
    $.easyAjax = function(options) {
        ...
        if (typeof opt.complete != "function") {
            opt.complete = function (jqXHR, textStatus) {
                ...
                if (opt.disableButton) {
                    if(opt.defaultTimeout) {
                        setTimeout(function(){
                            unloadingButton(opt.buttonSelector)
                        }, parseInt(opt.defaultTimeout));
                    } else {
                        unloadingButton(opt.buttonSelector)
                    }
                }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(opt.defaultTimeout)
                {
                    // alert(typeof opt.defaultTimeout);
                    setTimeout(function(){
        ...
        function loadingButton(selector) {
            ...
            var text = '<i class="fa fa-circle-o-notch fa-spin loading"></i>';

            if (button.width() < 20) {
                text = "...";
            }

            /* Don't use input, as we use .html() here to set text on button */
            if (!button.is("input")) {
    ...
    $.easyBlockUI = function (container, message) {
        ...
        var html = `<div> <div class="loader"> </div> </div>`;
